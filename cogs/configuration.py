...
class Configuration(commands.Cog):
    ...
    @config.command()
    async def ctf_category(self, ctx: commands.Context, category_name: str):
        """Set the category that new CTF channels are created in."""
    ...
    @commands.has_permissions(manage_channels=True)
    @config.command()
    async def archive_category(self, ctx: commands.Context, category_name: str):
        """Set the category that archived CTF channels are put in."""
        category_name = category_name.replace("$", "")
        category = discord.utils.get(ctx.guild.categories, name=category_name)
